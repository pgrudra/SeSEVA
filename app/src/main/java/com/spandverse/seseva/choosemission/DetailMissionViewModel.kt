...
class DetailMissionViewModel(mission: DomainActiveMission, application: Application) :
    ...
    private fun furtherTriggerText(contribution: Int, possibleMoney: Int) {
        if (contribution == 0) {
            ...
            spannable.setSpan(
                ...
                ), 19, 27 + possibleMoney.toString().length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
            )
            ...
            spannable.setSpan(
                RelativeSizeSpan(2f),
                10,
                14 + contribution.toString().length,
            ...
            _trigger.value = spannable
            val spannable2 =
                SpannableString("Your chance to add Rs $possibleMoney more\nbefore mission closes")
            spannable2.setSpan(
                ForegroundColorSpan(
                    ContextCompat.getColor(
                        context,
